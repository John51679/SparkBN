...
class BayesianNetwork() {
    ...
    def __calculateCPT__(vertexId: VertexId, data: DataFrame): Unit = {
        ...
        this.graph = this.graph.mapVertices((vid, tup) => if(vid == vertexId) {
            res
        }
        else {
            tup
    ...
    def __calculateMeanStd__(vertexId: VertexId, data: DataFrame): Unit = {
        ...
        if(edge.isEmpty()) {
            ...
            if(!discrete_data.columns.isEmpty) {
                ...
                val it = __combinationIterator__(lengths).toArray
                /* Point of interest for optimization */
                ...
                if(!cont_edges.isEmpty) {
                    cont_edges.foreach(ed => {
                        ...
                        this.graph = this.graph.mapEdges(g_ed => if(g_ed.srcId == ed.srcId) {
                            edges_res
                        }
                        else {
                            g_ed.attr
                      ...
                      *  */
                            ...
                            if(fields_arr(f._1.toInt).dataType == IntegerType) {
                                ...
                                val fetchedData = if(filtered.nonEmpty) {
                                    ...
                                    ("no_prefix", default_prob, Array.empty[(Double, Double)])
                                }
